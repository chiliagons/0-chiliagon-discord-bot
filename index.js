...
async function getpLength() {
  pdata = await proposal.fetchactiveProposals();
...
async function test() {
  await getpLength();
  data = await client.channels.cache.get("894205195557417021").messages.fetch();
  messagesCount = data.filter((msg) => msg.author.id == "889074690595885076");
  console.log(Array.from(messagesCount).length);
  if (Array.from(messagesCount).length + 1 == pLength) {
    client.channels.cache
      .get("894205195557417021")
      .send(`${proposalData[pLength - 1]}`);
  }
}
...
client.on("message", async (message) => {
  ...
  if (message.content == ".randomVolunteers") {
    ...
    message.guild.channels.cache.get(channelID).members.forEach((member) => {
      ...
      if (
        !member.user.bot &&
        member.user.username !== message.author.username
      ) {
    ...
    if (message.member.roles.cache.has(process.env.ROLE_ID)) {
      ...
      message.channel.send(
        `${ranppl[0]} , ${ranppl[1]} and ${ranppl[2]} chosen as volunteers! 🚀`
      );
      usernames = [];
    } else {
